...
```{r}
temp_file <- tempfile()
download.file("http://files.grouplens.org/datasets/movielens/ml-10m.zip", temp_file)
```
...
ratings <-
  fread(
    text = gsub("::", "\t", readLines(unzip(
      temp_file, "ml-10M100K/ratings.dat"
    ))),
...
```{r}
movies <-
  str_split_fixed(readLines(unzip(temp_file, "ml-10M100K/movies.dat")), "\\::", 3)
...
rm(temp_file, ratings, movies, test_index, temp, movielens, removed)
gc()
...
plot_genre <- data %>% group_by(genres) %>%
  summarize(
    n = n(),
